		...
		case 0:
			for(int i=0;i<4;i++){
				for(int j=28;j>-1;j=j-4)
					output[i]=output[i]|((_Sbox_0[(input[i]&(15<<j))>>j])<<j);
			}
			break;
		case 1:
			for(int i=0;i<4;i++){
				for(int j=28;j>-1;j=j-4)
					output[i]=output[i]|((_Sbox_1[(input[i]&(15<<j))>>j])<<j);
			}
			break;
		case 2:
			for(int i=0;i<4;i++){
				for(int j=28;j>-1;j=j-4)
					output[i]=output[i]|((_Sbox_2[(input[i]&(15<<j))>>j])<<j);
			}
			break;
		case 3:
			for(int i=0;i<4;i++){
				for(int j=28;j>-1;j=j-4)
					output[i]=output[i]|((_Sbox_3[(input[i]&(15<<j))>>j])<<j);
			}
			break;
		case 4:
			for(int i=0;i<4;i++){
				for(int j=28;j>-1;j=j-4)
					output[i]=output[i]|((_Sbox_4[(input[i]&(15<<j))>>j])<<j);
			}
			break;
	}
}
...
void Rev_SLayer(unsigned int input[4],unsigned int* output, int round){
	switch (round){
		case 0:
			for(int i=0;i<4;i++){
				for(int j=28;j>-1;j=j-4)
					output[i]=output[i]|((_Inv_Sbox_0[(input[i]&(15<<j))>>j])<<j);
			}
			break;
		case 1:
			for(int i=0;i<4;i++){
				for(int j=28;j>-1;j=j-4)
					output[i]=output[i]|((_Inv_Sbox_1[(input[i]&(15<<j))>>j])<<j);
			}
			break;
		case 2:
			for(int i=0;i<4;i++){
				for(int j=28;j>-1;j=j-4)
					output[i]=output[i]|((_Inv_Sbox_2[(input[i]&(15<<j))>>j])<<j);
			}
			break;
		case 3:
			for(int i=0;i<4;i++){
				for(int j=28;j>-1;j=j-4)
					output[i]=output[i]|((_Inv_Sbox_3[(input[i]&(15<<j))>>j])<<j);
			}
			break;
		case 4:
			for(int i=0;i<4;i++){
				for(int j=28;j>-1;j=j-4)
					output[i]=output[i]|((_Inv_Sbox_4[(input[i]&(15<<j))>>j])<<j);
			}
			break;
	}
}
...
void KeySchedule (unsigned int *key) {
	...
	for (int i=0; i<32 ; i++) 
		for (int j=0; j<4 ; j++) {
			...
			round_key[3][j]|= ((_Sbox_1[((prekey[8]&(1<<i))| ((prekey[9]&(1<<i))<<1) | ((prekey[10]&(1<<i))<<2) | ((prekey[11]&(1<<i))<<3)) & 15] & (1<<i)) & (1<<j));
			round_key[4][j]|= ((_Sbox_0[((prekey[12]&(1<<i))| ((prekey[13]&(1<<i))<<1) | ((prekey[14]&(1<<i))<<2) | ((prekey[15]&(1<<i))<<3)) & 15] & (1<<i)) & (1<<j));
			round_key[5][j]|= ((_Sbox_7[((prekey[16]&(1<<i))| ((prekey[17]&(1<<i))<<1) | ((prekey[18]&(1<<i))<<2) | ((prekey[19]&(1<<i))<<3)) & 15] & (1<<i)) & (1<<j));
